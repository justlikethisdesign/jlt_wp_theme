 ...
 * This is obtained using the JLT_get_semantics function
 *
 * @param string $id the class identifier
 */
...
if ( ! function_exists( 'jlt_posted_on' ) ) {

	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */

	function jlt_posted_on() {
		printf( __( '<p><time class="entry-date updated published dt-updated dt-published" datetime="%1$s" itemprop="dateModified datePublished">%2$s</time></p>', jlt_text_domain() ),
...
if ( ! function_exists( 'jlt_content_nav' ) ) {
	...
	function jlt_content_nav( $nav_id, $button_text = null ) {
		...
		?>
            <nav id="<?php echo esc_attr( $nav_id ); ?>">
                ...
                <h1 class="assistive-text section-heading screen-reader-text"><?php _e( 'Post navigation', jlt_text_domain() ); ?></h1>

                <?php if ( is_single() ) { // navigation links for single posts

                    the_posts_pagination( array(
                        'prev_text' => '<span class="screen-reader-text"><i class="fa fa-angle-left" aria-hidden="true"></i> ' . __( 'Previous post', jlt_text_domain() ) . '</span>',
                        'next_text' => '<span class="screen-reader-text">' . __( 'Next post', jlt_text_domain() ) . '</span> <i class="fa fa-angle-right" aria-hidden="true"></i>',
                        'before_page_number' => '<span class="meta-nav screen-reader-text">' . __( 'Post', jlt_text_domain() ) . ' </span>',
                    ) );
                    ...
                    <div id="load-more-wrap">
                        ...
                        **/
                        ...
                        $load_type = 'paging';
                        if ( get_theme_mod('jlt_loading_type') ) {
                            $load_type = get_theme_mod('jlt_loading_type');
                        }
                        ...
                        if( !$button_text ){
                            $button_text = __( 'Load More', jlt_text_domain() );
                        }
                        ...
                        <?php if ( $load_type != 'paging' ) { ?>
                            ...
                            <div id="load-more-posts-error" class="load-more-posts-error error smooth"><p><?php echo esc_html__( 'Something has gone wrong. Please try again.', jlt_text_domain() );?></p></div>
                            <button id="load-more-posts" class="load-more-posts-button" data-paged="<?php echo esc_attr__( $paged, jlt_text_domain() );?>" data-query='<?php echo json_encode ( $wp_query->query ) ;?>' data-custom-template="<?php echo $template;?>" data-loadtype="<?php echo $load_type;?>" style="display: none;"><?php echo $button_text;?></button>
 ...
 * @since JLT 1.0.0
 */
function jlt_the_post_thumbnail( $before = '', $after = '' ) {
 ...
 * @since JLT 1.0.0
 */
function jlt_the_article_banner( $before = '', $after = '' ) {
	if ( '' != get_the_post_thumbnail() ) {
...
/**
 * Adds card thumbnail support
 *
 * @since JLT 1.0.0
 */
function jlt_the_card_image() {
	if ( '' != get_the_post_thumbnail() ) {
		echo '<div class="entry-media"><div class="media-fit">';
        echo '<div class="overlay smooth"></div>';
		the_post_thumbnail( 'card', array( 'class' => 'photo u-photo u-featured obj-fit cover smooth', 'itemprop' => 'image' ) );
		echo '</div></div>';
	};
}
...
/**
 * Adds card gallery thumbnail support
 *
 * @since JLT 1.0.0
 */
function jlt_the_card_video_image() {
	...
	if ( !$placeholder ) {
        $placeholder = get_the_post_thumbnail( get_the_ID(), 'card', array( 'class' => 'photo u-photo u-featured obj-fit cover smooth', 'itemprop' => 'image' ) );
	};
...
/**
 * Adds card gallery support
 *
 * @since JLT 1.0.0
 */
function jlt_the_card_gallery_images() {
    ...
    foreach ( $gallery_ids as $image_id ){
        ...
        $image = wp_get_attachment_image( $image_id, 'card', array( 'class' => 'photo u-photo', 'itemprop' => 'image' ) );
...
/**
 * Builds the footer for cards
 */

function jlt_interaction_bar( $panel = null ){
...
function jlt_social_share( $post_id ){

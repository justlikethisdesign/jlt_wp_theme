...
add_filter( 'gform_ajax_spinner_url', 'jlt_custom_g_forms_spinner' );
...
function jlt_custom_g_forms_spinner( $src ) {
...
function jlt_gravity_form_exists($form_id){
...
/**
 * Make sure the gravity forms campaign submittion is saved
 * so the user doesnt have to re-add details every time
 **/ 

add_action("gform_after_submission_6", "after_submission_6", 10, 2);
function after_submission_6($entry, $form){
    setcookie('download', 1, 2147483647);
}

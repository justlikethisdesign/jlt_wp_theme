 ...
 * @since JLT 1.0.0
 */
function jlt_get_file_version_number( $name, $file ){
    
    $timestamp = get_file_last_mod_stamp( $file );
    
    if( $name && $timestamp ){
        
        $version_num = jlt_set_file_version_number( $name, $timestamp );
 ...
 * @since JLT 1.0.0
 */
function jlt_set_file_version_number( $name, $timestamp ){
    
    $version_option_name = 'jlt' . $name . '_version_number';
    ...
    //jlt_inc_version_number can take an empty string, or a 3 point number
    $new_version = jlt_inc_version_number( $current_version );
 ...
 * @since JLT 1.0.0
 */
function jlt_inc_version_number( $version = null ){
 ...
 * @since JLT 1.0.0
 */
function jlt_check_modif_date($timestamp, $old_stamp){
